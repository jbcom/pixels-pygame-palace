...
class ResponsiveOverflowTest {
     ...
     */
    async checkForHorizontalOverflow() {
        if (!this.driver)
     ...
     */
    async testResolution(name, dimensions) {
        if (!this.driver)
            throw new Error('Driver not initialized');
        ...
        await this.driver.wait(selenium_webdriver_1.until.elementLocated(selenium_webdriver_1.By.css('body')), 5000);
        ...
        if (overflowCheck.hasOverflow) {
            ...
            throw new Error(`Horizontal overflow detected at ${name} (${dimensions.width}x${dimensions.height})`);
        }
        else {
        ...
        try {
            ...
            const optionButtons = await this.driver.findElements(selenium_webdriver_1.By.css('[data-testid*="dialogue-option"]'));
            if (optionButtons.length > 0) {
                ...
                if (optionOverflowCheck.hasOverflow) {
                    console.error(`  ❌ OVERFLOW in dialogue options at ${dimensions.width}x${dimensions.height}`);
     ...
     */
    async testProblematicResolutions() {
